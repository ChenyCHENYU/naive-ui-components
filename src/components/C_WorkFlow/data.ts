...
/* 默认头像生成函数：使用本地 data URL，避免将姓名发送给第三方服务。 */
export const getDefaultAvatar = (name: string): string => {
  ...
  const hue = Array.from(name).reduce(
    (value, char) => (value * 31 + (char.codePointAt(0) ?? 0)) % 360,
    210
  )
  const safeInitials = initials.replace(
    /[&<>"']/g,
    char =>
      ({
        '&': '&amp;',
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 80 80"><rect width="80" height="80" rx="40" fill="hsl(${hue} 55% 46%)"/><text x="40" y="48" text-anchor="middle" font-family="sans-serif" font-size="28" fill="white">${safeInitials}</text></svg>`
